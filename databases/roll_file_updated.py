...
# using pickle to store date time
...
db.execute("CREATE TABLE IF NOT EXISTS history (time TIMESTAMP NOT NULL,"
           "account TEXT NOT NULL, amount INTEGER NOT NULL, zone INTEGER NOT "
           "NULL, PRIMARY KEY (time, account))")

db.execute("CREATE VIEW IF NOT EXISTS local_history AS SELECT "
...
class Account(object):
    ...
    def _save_update(self, amount):
        ...
        deposit_time, zone = Account._current_time()  # <----unpack the
        # returned tuple
        ...
        db.execute("INSERT INTO history VALUES(?, ?, ?, ?)",
                   (deposit_time, self.name, amount, pickle_zone))
        db.commit()
    ...
    def withdraw(self, amount: int) -> float:
        if 0 < amount <= self._balance:
            ...
            print(
                "The amount must be greater than zero and no more than your "
                "account balance")
            return 0.0
